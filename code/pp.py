...
def get_next_batch(data_set, prev, batch_size):
    ...
    for row in batch:
        ...
        article = [
            l.replace('  \n', ' ').replace('  #@NEW_LINE#@#', '').replace('title', '') for l in article]
        article = [l for l in article if len(l) > 2]

        truncated_article = []
        num_lines_article = len(article)

        for i, line in enumerate(article):
            words = nltk.tokenize.word_tokenize(line)
            num_words = len(words)

            if num_words == 1 and i < num_lines_article - 1: # This is a section heading
                next_line_words = nltk.tokenize.word_tokenize(article[i + 1])
                new_line = next_line_words[:min(256, len(next_line_words))]
                truncated_article.append(' '.join(new_line))
                

        article = ' '.join(truncated_article)
        # article = ''.join(article[1:])

        summary_words = nltk.tokenize.word_tokenize(summary)
        summary_words = summary_words[:min(784, len(summary_words))]
        
        summary = ' '.join(summary_words)

        articles_batch.append(article.lower())
        summaries_batch.append(summary.lower())
